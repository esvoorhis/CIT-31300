...
class Post extends Model{
    ...
    function getComment ($commentID){
        $sql = 'Select c.commentID, c.commentText, c.date, c.postID, c.uID FROM comments c 
        inner join users u on u.uid = c.uid
        inner join posts p on p.pid = c.postid
        where p.pID = ?
        ';
        $results = $this->getrow($sql, array($commentID));
        $comment = $results;
        return $comment;
    }
    ...
    public function addComment($data){
        $sql = 'insert into comments (commentID,commentText, date, postID, uID) values (?,?,?,?,?)';
        $this->db->execute($sql,$data);
